...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const isValidEmail = (email) => {
    ...
    const domain = email.toLowerCase().split('@')[1];
    return typeof allowedEmailDomains !== "undefined"
      ? allowedEmailDomains.includes(domain)
      : true; // если массива нет, пропускает
  };

  // === Универсальная валидация всей формы
  ...
  // === Ограничения ввода и проверки валидности на лету
  nameInput.addEventListener("input", () => {
    nameInput.value = nameInput.value.replace(/[^a-zA-Zа-яА-ЯёЁіІїЇєЄґҐ -]/g, "");
    checkFormValidity();
  });
